...
**Last shipped:** Phase J randomized cadence + self-rescheduling chain
deployed live (2026-05-15, HEAD `66c9132`). Beat schedule swap: drop
fixed `scan-all-sources` 30-min, add `scan-watchdog` 5-min safety net.
`scan_all_sources` self-reschedules with `random.uniform(600, 1500)`s
countdown via `_enqueue_next_scan()`. Full suite **741 passed** (+26).

**Phase I + J progress:**
- ✅ I-A Per-account fingerprint pinning
- ✅ I-B Cookie rotation capture
- ✅ I-C Persistent browser profile
- ✅ I-D Scanner cadence (interval + jitter + think-time)
- ✅ I-E Stealth patch
- ✅ J-1 Randomized cadence config knobs + watchdog beat schedule
- ✅ J-2 Self-rescheduling `_enqueue_next_scan` + task wire
- ✅ J-3 `scan_watchdog` stale-chain detection
- ✅ J-4/J-5 Deploy + manual login + verification

**Verification post-deploy (2026-05-15 02:43 UTC):**
- ScannerRun 320 manual: `inserted=3, errors=0` in 73.4s ✓
- Selfsched fired: `next in 689.2s (11.5 min) trigger=selfsched` ✓
- Watchdog fired: ScannerRun 319 kicked at 02:40 UTC (idle > 1800s) ✓
- 3 services active, profile clean, no Singleton lock leftover

**Observation #5 baseline:** T0=2026-05-15 02:38 UTC (manual login via
noVNC display:3 + Playwright Chromium binary). Cookie expiry 1-year
(2027-05-15). Source id=1 only (home_feed); group id=2 still disabled.
Target survival > 5h (baseline) to validate Phase J cadence helps.

**Next step:** Observe + watch for first selfsched-driven cookie flip.
If survival > obs#4 (1.5h) we get directional signal Phase J helps.
If still ~1.5h, account-specific flag confirmed → Phase K = swap account.

## Activity Log

| Tanggal | Status | Summary |
|---------|--------|---------|
| 2026-05-15 | phase J randomized cadence + selfsched chain deploy live + obs#5 baseline | **Phase J** ship via TDD strict (J-1/J-2/J-3 RED→GREEN, 7 commits). **J-1** `bot/celery_app.py` `_scan_min_interval()` (env `SCAN_MIN_INTERVAL_SECONDS` default 600s) + `_scan_max_interval()` (env `SCAN_MAX_INTERVAL_SECONDS` default 1500s) + remove `scan-all-sources` beat entry + add `scan-watchdog` every 300s. **J-2** `bot/tasks.py` +`_enqueue_next_scan(*, source='selfsched')` helper using `random.uniform(MIN, MAX)` countdown via `scan_all_sources.apply_async(kwargs={'trigger': source})`, env kill-switch `SCAN_SELFSCHED_DISABLED=1` → noop, wire ke finally-block `scan_all_sources` (fires even on exception), trigger whitelist expand `('beat','manual','selfsched','watchdog')`. **J-3** +`@app.task scan_watchdog()` end-of-file: query latest `ScannerRun`, if no history OR finished_at NULL (mid-flight) OR idle > `SCAN_MAX_IDLE_SECONDS` (default 1800) → kick `_enqueue_next_scan(source='watchdog')` + return `{action: 'kicked', reason, idle_seconds}`. Naive datetime coercion (assume UTC if tzinfo missing). **Tests:** 26 new (13 celery_schedule + 10 scan_all_sources self-resched + 7 watchdog stale-chain). Full suite **741 passed**. **Manual login launcher** `scripts/manual_login.py` (3969 bytes, headless=False on `DISPLAY=:3`, persistent profile reuse via `create_persistent_session`). **Deploy 02:25 UTC:** server `git fetch + merge --ff-only origin/main` HEAD `66c9132`, `sudo systemctl restart fb-bot-worker fb-bot-beat` all active, beat schedule live: `scan-watchdog` 300s + `collect-all-targets` 1800s + `health-check` 300s, no `scan-all-sources` ✓. **Manual login obs#5 (T0=02:38 UTC):** old account flipped EXPIRED at T+1.93h obs#4 (Phase I-D ga cukup), launch `manual_login.py` background, user login via noVNC `localhost:6083/vnc.html` + ssh tunnel, c_user/xs cookies persisted 1-year (expires 2027-05-15), profile flush bersih (Cookies file mtime 02:39 UTC, 20480 bytes). DB reset `status=ACTIVE failure_count=0 cookies_expired_at=NULL`. **Verify:** ScannerRun 320 manual `inserted=3 errors=0 73.4s` ✓ + log `scan self-rescheduled: next in 689.2s (11.5 min) trigger=selfsched` ✓ + ScannerRun 319 watchdog kicked at 02:40 UTC (idle > 1800s sejak 01:06) ✓. Phase J end-to-end working. Cron probe obs#4 stopped (`13235d36268c`). **Outcome obs#5 pending** — if survival > 5h baseline, Phase J cadence helps; if still ~1.5h, account-flag terminal → Phase K (account swap). HEAD `66c9132`. |
