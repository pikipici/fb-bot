...
| 2026-05-10 | single-account fb-accounts + credentials fix | Enforce 1-account limit: `POST /fb-accounts` return 409 kalau udah ada akun (any status, termasuk BLOCKED/DISABLED). Add `GET /fb-accounts/current` (null kalau kosong). Router test: 20/20 pass (new `TestSingleAccountLimit` + `TestGetCurrentAccount`). Full suite 294 passed. Frontend: refactor FBAccounts.tsx jadi single-account UX — empty state Setup card, populated state detail card + Edit/Delete/Reactivate, TANPA tombol Add duplicate. Field `purpose` disembunyin dari form (tetep kirim `both` default). Dashboard build 1725 modules 799ms, 443KB js/137KB gz. Ops fix: `CREDENTIALS_KEY` gak di-set di `.env` → ephemeral key regenerate tiap restart → `InvalidToken` decrypt. Fix: generate Fernet key, append ke `.env`, tetap `ENV=development` (SSH tunnel access, CORS_ORIGINS gak ke-set). Delete orphan record sisa uji. HEAD `af9535b`. |
